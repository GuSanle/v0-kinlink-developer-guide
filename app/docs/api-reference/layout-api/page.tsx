import { CodeBlock } from "@/components/code-block"

export default function LayoutApiPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="scroll-m-20 text-4xl font-bold tracking-tight">布局API</h1>
        <p className="mt-2 text-lg text-muted-foreground">用于控制表单布局和UI元素的函数。</p>
      </div>

      <div className="docs-content">
        <p>
          布局API（<code>kinlink.layoutApi</code>
          ）提供了用于控制表单布局元素（如页眉、页脚和操作栏）的函数。此API帮助您自定义Kinlink表单的外观和行为，特别是针对不同设备类型。
        </p>

        <h2>页眉控制</h2>

        <h3>getHeaderHeight()</h3>
        <p>获取页眉元素的当前高度。</p>
        <CodeBlock
          code={`// 获取页眉的高度
const headerHeight = kinlink.layoutApi.getHeaderHeight();
console.log(headerHeight); // 输出: 64 (像素)`}
          language="javascript"
        />

        <h3>isHeaderVisible()</h3>
        <p>检查页眉元素当前是否可见。</p>
        <CodeBlock
          code={`// 检查页眉是否可见
const isVisible = kinlink.layoutApi.isHeaderVisible();
console.log(isVisible); // 输出: true 或 false`}
          language="javascript"
        />

        <h3>hideHeader()</h3>
        <p>隐藏页眉元素。</p>
        <CodeBlock
          code={`// 隐藏页眉
kinlink.layoutApi.hideHeader();`}
          language="javascript"
        />

        <h3>showHeader()</h3>
        <p>显示先前隐藏的页眉元素。</p>
        <CodeBlock
          code={`// 显示页眉
kinlink.layoutApi.showHeader();`}
          language="javascript"
        />

        <h2>页脚控制</h2>

        <h3>getFooterHeight()</h3>
        <p>获取页脚元素的当前高度。</p>
        <CodeBlock
          code={`// 获取页脚的高度
const footerHeight = kinlink.layoutApi.getFooterHeight();
console.log(footerHeight); // 输出: 48 (像素)`}
          language="javascript"
        />

        <h3>isFooterVisible()</h3>
        <p>检查页脚元素当前是否可见。</p>
        <CodeBlock
          code={`// 检查页脚是否可见
const isVisible = kinlink.layoutApi.isFooterVisible();
console.log(isVisible); // 输出: true 或 false`}
          language="javascript"
        />

        <h3>hideFooter()</h3>
        <p>隐藏页脚元素。</p>
        <CodeBlock
          code={`// 隐藏页脚
kinlink.layoutApi.hideFooter();`}
          language="javascript"
        />

        <h3>showFooter()</h3>
        <p>显示先前隐藏的页脚元素。</p>
        <CodeBlock
          code={`// 显示页脚
kinlink.layoutApi.showFooter();`}
          language="javascript"
        />

        <h2>提交按钮控制</h2>

        <h3>isSubmitButtonVisible()</h3>
        <p>检查提交按钮当前是否可见。</p>
        <CodeBlock
          code={`// 检查提交按钮是否可见
const isVisible = kinlink.layoutApi.isSubmitButtonVisible();
console.log(isVisible); // 输出: true 或 false`}
          language="javascript"
        />

        <h3>hideSubmitButton()</h3>
        <p>隐藏提交按钮。</p>
        <CodeBlock
          code={`// 隐藏提交按钮
kinlink.layoutApi.hideSubmitButton();`}
          language="javascript"
        />

        <h3>showSubmitButton()</h3>
        <p>显示先前隐藏的提交按钮。</p>
        <CodeBlock
          code={`// 显示提交按钮
kinlink.layoutApi.showSubmitButton();`}
          language="javascript"
        />

        <h2>移动端特定控制</h2>

        <p>Kinlink为移动设备提供了特殊控制。这些对于在小屏幕上创建更好的用户体验特别有用。</p>

        <h3>checkIsMobileDevice()</h3>
        <p>检查当前设备是否为移动设备（视口宽度 ≤ 575px）。</p>
        <CodeBlock
          code={`// 检查当前设备是否为移动设备
if (kinlink.layoutApi.checkIsMobileDevice()) {
  // 应用移动端特定的自定义设置
  console.log('在移动设备上运行');
} else {
  console.log('在桌面设备上运行');
}`}
          language="javascript"
        />

        <h3>mobileIsFormActionBarVisible()</h3>
        <p>检查移动表单操作栏当前是否可见。</p>
        <CodeBlock
          code={`// 检查移动表单操作栏是否可见
const isVisible = kinlink.layoutApi.mobileIsFormActionBarVisible();
console.log(isVisible); // 输出: true 或 false`}
          language="javascript"
        />

        <h3>mobileHideFormActionBar()</h3>
        <p>隐藏移动表单操作栏。</p>
        <CodeBlock
          code={`// 隐藏移动表单操作栏
kinlink.layoutApi.mobileHideFormActionBar();`}
          language="javascript"
        />

        <h3>mobileShowFormActionBar()</h3>
        <p>显示先前隐藏的移动表单操作栏。</p>
        <CodeBlock
          code={`// 显示移动表单操作栏
kinlink.layoutApi.mobileShowFormActionBar();`}
          language="javascript"
        />

        <h3>toggleFormActionBar()</h3>
        <p>切换移动表单操作栏的可见性。</p>
        <CodeBlock
          code={`// 切换移动表单操作栏
kinlink.layoutApi.toggleFormActionBar();`}
          language="javascript"
        />

        <h3>mobileGetFormActionBarHeight()</h3>
        <p>获取移动表单操作栏的当前高度。</p>
        <CodeBlock
          code={`// 获取移动表单操作栏的高度
const height = kinlink.layoutApi.mobileGetFormActionBarHeight();
console.log(height); // 输出: 70 (像素)`}
          language="javascript"
        />

        <h2>通用布局工具</h2>

        <h3>getContentAreaHeight()</h3>
        <p>计算表单内容区域的可用高度，考虑页眉、页脚和操作栏的高度。</p>
        <CodeBlock
          code={`// 获取可用内容区域高度
const contentHeight = kinlink.layoutApi.getContentAreaHeight();
console.log(contentHeight); // 输出: 600 (像素)`}
          language="javascript"
        />

        <h3>onLayoutChange(callback)</h3>
        <p>注册一个回调函数，在布局变化时调用（例如，当页眉、页脚或操作栏显示或隐藏时）。</p>
        <CodeBlock
          code={`// 注册布局变化监听器
const cleanup = kinlink.layoutApi.onLayoutChange((detail) => {
  console.log('布局已变化:', detail);
  // detail = { element: 'header', visible: false, height: 0 }
  
  // 根据变化重新计算布局
  const contentHeight = kinlink.layoutApi.getContentAreaHeight();
  console.log('新内容高度:', contentHeight);
});

// 稍后，当不再需要监听器时
cleanup();`}
          language="javascript"
          showLineNumbers={true}
        />

        <h2>示例：响应式布局</h2>

        <p>以下是如何使用布局API创建适应不同设备类型的响应式表单的示例：</p>

        <CodeBlock
          code={`(function() {
  kinlink.events.on(kinlink.FormEvents.FORM_LOADED, () => {
    const isMobile = kinlink.layoutApi.checkIsMobileDevice();
    
    if (isMobile) {
      // 移动布局优化
      
      // 隐藏页眉以节省空间
      kinlink.layoutApi.hideHeader();
      
      // 确保移动操作栏可见
      kinlink.layoutApi.mobileShowFormActionBar();
      
      // 监听方向变化
      window.addEventListener('resize', () => {
        const contentHeight = kinlink.layoutApi.getContentAreaHeight();
        console.log('调整大小后的内容高度:', contentHeight);
        
        // 根据需要调整布局
        // ...
      });
    } else {
      // 桌面布局
      
      // 显示页眉和页脚
      kinlink.layoutApi.showHeader();
      kinlink.layoutApi.showFooter();
      
      // 监听布局变化
      kinlink.layoutApi.onLayoutChange((detail) => {
        console.log('布局元素已变化:', detail);
        
        // 根据需要调整布局
        // ...
      });
    }
  });
})();`}
          language="javascript"
          filename="responsive-layout.js"
          showLineNumbers={true}
        />

        <h2>最佳实践</h2>

        <ul>
          <li>
            <strong>首先检查设备类型：</strong> 在应用移动端特定优化之前，始终检查设备是否为移动设备。
          </li>
          <li>
            <strong>监听布局变化：</strong> 使用<code>onLayoutChange</code>回调动态响应布局变化。
          </li>
          <li>
            <strong>计算可用空间：</strong> 使用<code>getContentAreaHeight</code>确定表单内容可用的空间。
          </li>
          <li>
            <strong>清理监听器：</strong> 记得清理注册的任何监听器，以防止内存泄漏。
          </li>
        </ul>
      </div>
    </div>
  )
}
