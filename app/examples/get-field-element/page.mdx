...
<div className="container py-6 lg:py-10">
  <div className="flex items-center gap-4 mb-8">
    ...
    <h1 className="text-3xl font-bold tracking-tight">DOM元素访问控制示例</h1>
  </div>

  <div className="my-8 space-y-6">
    <div className="space-y-4">
      <h2 className="text-2xl font-bold tracking-tight">元素访问机制概览</h2>
      > 直接操作表单DOM元素，实现高级样式定制和原生事件处理

      此示例演示如何使用**kinlink API**获取表单字段的**DOM元素**，进行更精细的控制和定制。通过直接访问原生HTML元素，实现**高级样式**、**自定义事件监听**和**第三方库集成**。

      ### DOM访问核心能力
      - 🎯 **精确元素定位**：获取特定字段的DOM元素引用
      - 🎨 **原生样式控制**：直接操作CSS样式和类名
      - 👂 **事件监听增强**：添加自定义原生事件处理
      - 🔧 **属性动态管理**：设置和修改HTML元素属性
      - 📦 **第三方集成**：与外部UI库和插件无缝集成
      - ⚡ **性能优化**：避免重复查询，缓存元素引用
    </div>
